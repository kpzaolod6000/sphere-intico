<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" dir="{{ company_setting('site_rtl',$company_id,$workspace) == 'on'?'rtl':''}}">
<meta name="csrf-token" id="csrf-token" content="{{ csrf_token() }}">
<head>

<title>@yield('page-title') | {{ !empty(company_setting('title_text',$company_id,$workspace)) ? company_setting('title_text',$company_id,$workspace) : (!empty(admin_setting('title_text')) ? admin_setting('title_text') :'WorkDo') }}</title>
...
<meta content="width=device-width, initial-scale=1, maximum-scale=1, shrink-to-fit=no" name="viewport">
 ...
 @if (company_setting('site_rtl',$company_id,$workspace) == 'on')
        ...
        @endif
        @if(company_setting('cust_darklayout',$company_id,$workspace) == 'on')
...
<body class="{{ !empty(company_setting('color',$company_id,$workspace))?company_setting('color',$company_id,$workspace):'theme-3' }}">
    <div class="container">
