...
<head>
    ...
    <title>{{Modules\Sales\Entities\Quote::quoteNumberFormat($quote->quote_id,$quote->created_by,$quote->workspace)}} | {{ !empty(company_setting('title_text',$quote->created_by,$quote->workspace)) ? company_setting('title_text',$quote->created_by,$quote->workspace) : (!empty(admin_setting('title_text')) ? admin_setting('title_text') :'WorkDo') }}</title>
    <link
        ...
        <div class="invoice-header">
            <table class="vertical-align-top">
                <tbody>
                    <tr>
                        <td >
                            <h3 style="text-transform: uppercase; font-size: 30px; font-weight: bold; margin-bottom: 10px; color: {{$color}};">{{ __('Quote')}}</h3>
                            <p>
                            ...
                            <div style="justify-content: end; display: flex;">
                                ...
                                <table class="no-space">
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{ __('Number: ')}}</td>
                                            <td class="text-right">{{Modules\Sales\Entities\Quote::quoteNumberFormat($quote->quote_id,$quote->created_by,$quote->workspace)}}</td>
                                        </tr>
                                        <tr>
                                            <td>{{ __('Issue Date:')}}</td>
                                            <td class="text-right">{{company_date_formate($quote->issue_date,$quote->created_by,$quote->workspace)}}</td>
                                        </tr>
                        ...
                        <td colspan="2" class="sub-total">
                            <table class="total-table">
                                @if($quote->getTotalDiscount())
                                    <tr>
                                        <td>{{__('Discount')}}:</td>
                                        <td>{{currency_format_with_sym($bill->getTotalDiscount(),$bill->created_by)}}</td>
                                    </tr>
                                @endif
