...
<head>
    ...
    <title>{{Modules\Sales\Entities\SalesInvoice ::invoiceNumberFormat($invoice->invoice_id,$invoice->created_by,$invoice->workspace)}} | {{ !empty(company_setting('title_text',$invoice->created_by,$invoice->workspacey)) ? company_setting('title_text',$invoice->created_by,$invoice->workspacey) : (!empty(admin_setting('title_text')) ? admin_setting('title_text') :'WorkDo') }}</title>
    <link
        ...
        <div class="invoice-header">
            <table class="vertical-align-top">
                <tbody>
                    <tr>
                        <td >
                            <h3 style="text-transform: uppercase; font-size: 40px; font-weight: bold; color: var(--theme-color); margin-bottom: 10px;">{{ __('Sales Invoice')}}</h3>
