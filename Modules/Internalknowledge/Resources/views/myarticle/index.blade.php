                                            ...
                                            <span>
                                                ...
                                                {{-- @can('article show')
                                                    <div class="action-btn bg-warning ms-2">
                                                        <a target="_blank" href="{{ route('mindmap.index', $article->id) }}"
                                                            data-bs-toggle="tooltip"
                                                            data-title="{{ __('Article Details') }}"title="{{ __('Details') }}"
                                                            class="mx-3 btn btn-sm d-inline-flex align-items-center text-white" id="save">
                                                            <i class="ti ti-eye"></i>
                                                        </a>
                                                    </div>
                                                @endcan --}}
...
{{-- <script>
    ...
    $(document).on('click', '#save', function(e) {
        ...
        // if (articleId) {
            $.ajax({
                url: '{{ route('mindmap.show', ['id' => $article->id]) }}',
                method: 'POST',
                ...
                success: function(response) {
                    ...
                    const getApi =
                        'http://localhost/product/workdo-dash/internalknowledge/article/mindmap/' +
                        articleId + '/?k=' + key;
